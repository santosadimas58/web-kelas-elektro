...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $title ?? 'Dashboard' }} | {{ $sharedSiteSetting->site_name ?? config('classroom.site_name') }}</title>
        <meta
            name="description"
            content="{{ $sharedSiteSetting->tagline ?? 'Dashboard pengelolaan website kelas.' }}"
        >
